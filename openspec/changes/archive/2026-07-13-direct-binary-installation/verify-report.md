...
ok  github.com/dnieblesdev/dniebles-bootstrap/cmd/dbootstrap  0.116s  coverage: 94.3% of statements
```
...
| Install and validate managed payloads | Existing files are protected | `install_test.sh > test_unmanaged_file_refused` (aborts, no overwrite, no partial) | ✅ COMPLIANT |
...
Extra triangulation (beyond spec scenarios): `test_exact_version` (R1 "accepts vX.Y.Z"), `test_force_required_for_managed_reinstall` (R3 "require --force; force permits upgrade/downgrade"), PATH guidance runtime output captured in apply-progress.
...
| Atomic replace + backups + transaction recovery | ✅ Implemented | `cp` to `.tmp` then `mv`; `test_checksum_mismatch` confirms nothing left behind |
...
| `--force` for matching managed install | ✅ Implemented | `test_force_required_for_managed_reinstall` |
...
| Existing files: state manifest, `--force`, abort on unmanaged | ✅ Yes | 3 shell scenarios |
...
| All tasks have tests | ✅ | 11/11 (3 Go + 10 shell scenarios) |
| RED confirmed (tests exist) | ✅ | install_test.sh + main_test.go exist |
| GREEN confirmed (tests pass) | ✅ | All pass on fresh execution |
| Triangulation adequate | ✅ | 10 shell scenarios + multiple Go subtests with distinct expected values |
...
| Shell/Integration | 10 scenarios | `install_test.sh` | bash + tar + sha256sum |
...
|------|-----|-------|-------------|------------|--------------|
| 1.1-1.3 (shell) | Written | Passed | 10 scenarios | N/A (new) | ✅ install_test.sh passes |
| 2.1-2.2 (shell) | Written | Passed | safe uninstall + modified preservation | N/A (new) | ✅ passes |
...
All 11 tasks complete; all 6 spec scenarios have covering tests that passed on fresh execution (shell + Go, plus a freshly built binary reading the XDG catalog from an arbitrary CWD); build, vet, format, and full `go test ./...` green; design decisions followed with two disclosed non-breaking deviations; TDD compliance complete. The change is ready to archive.